...
parser.add_argument('token',
                    help='Access token used to authenticate yourself when making API requests.')
...
parser.add_argument('-g', '--group',
                    help='Name of group to retrieve. Run without this flag to see list of groups.')
...
def getMessages(group_id, direct_msgs, before_id=None, since_id=None):
    ...
    try:
        if direct_msgs:
            ...
            msgs = get(
                requests.get(URL + '/groups/' + group_id + '/messages' + TOKEN, params=params))
...
def main(retrieve_all, direct_msgs, group_name, csv_file, overwrite):
    ...
    if retrieve_all:
        ...
        if group_name not in groups:
            ...
            count, _ = countMsgs(group_name, groups[group_name]['id'], direct_msgs,
                                 csv_file=csv_file)
            print "Processed {} messages. Wrote to {}.".format(count, csv_file)
    else:
